function Find-BrowserExtensions {
    ...
    [String]
    $FindExtensionConfig = $null,

    [Parameter(Position = 4)]
    [String]
    $crxCavatorReport = $null
    ...
    function New-CRXCavatorOut 
    {
        ...
        [string]
        $ExtensionCSPPolicy = $null,

        [string]
        $RiskTotalScore = $null,

        [string]
        $VulnerablitySummary = $null,

        [string]
        $VulnerablityCVE = $null,

        [string]
        $VulnerablitySeverity = $null,

        [string]
        $VulnerabiltyInfo = $null
    ...
    param(
        ...
        try 
        {
            $UserPaths = (Get-WmiObject win32_userprofile -ComputerName $Computer -ErrorAction Stop | Where-Object localpath -notmatch 'Windows' ).localpath
        }
        ...
        foreach ($User in $Users) 
        {
            ...
            $EdgeExt = Get-ChildItem -Path "\\$($Computer)\c$\Users\$($User)\AppData\Local\Microsoft\Edge\User Data\Default\Extensions\" -ErrorVariable onError -ErrorAction SilentlyContinue

            if ($onError) 
            {
                $e =  $onError[0].TargetObject
            }
                ...
                foreach ($verEdgeFolder in $varEdgeFolders) 
                {
                    ...
                    if (Test-Path -Path ($verEdgeFolder.FullName + '\manifest.json')) 
                    {
                            ...
                            @('\_locales\en_US\', '\_locales\en\') | % {
                                ...
                                if (Test-Path -Path ($verEdgeFolder.Fullname + $_ + 'messages.json')) 
                                {
                                    $EdgeAppManifest = Get-Content ($verEdgeFolder.Fullname + $_ +'messages.json') | ConvertFrom-Json
                                        @($EdgeAppManifest.appName.message, $EdgeAppManifest.extName.message, $EdgeAppManifest.extensionName.message, $EdgeAppManifest.app_name.message, $EdgeAppManifest.application_title.message, $EdgeAppManifest.$EdgeAppId.message)`
                                        | % { if (($_) -and (-not($EdgeExtName))) 
                                                {
                                                    $EdgeExtName = $_
                                                }
                                            }
                        ...
                        if ($($EdgeFolder.name) -notin $script:config.Edge.ExtensionId) 
                        {
                            ...
                            $OutputSend = New-BrowserOutput -Workstation $Computer -Browser $Browser -ExtensionName $EdgeExtName -ExtensionVersion $EdgeManifest.version -ExtensionId $EdgeFolder.name -ContentScripts $($EdgeManifest.content_scripts.js) `
                            -ContentSecurityPolicy $($EdgeManifest.content_security_policy) -Permissions $($EdgeManifest.permissions)
                    ...
                    if (Test-Path -Path ($verFolder.FullName + '\manifest.json'))
                    {
                        ...
                        if ($Manifest.name -like '__MSG*') 
                        {
                            ...
                            if (Test-Path -Path ($verFolder.Fullname + $_ + 'messages.json')) 
                            {
                                ...
                                @($AppManifest.appName.message, $AppManifest.extName.message, $AppManifest.extensionName.message, $AppManifest.app_name.message, $AppManifest.application_title.message, $AppManifest.$AppId.message)`
                                | % { 
                                    if (($_) -and (-not($ExtName))) { $ExtName = $_ }
                                }
                            }
                        ...
                        {
                            $OutputSend = New-BrowserOutput -Workstation $Computer -Browser $Browser -ExtensionName $ExtName -ExtensionVersion $Manifest.version -ExtensionId $Folders.name -ContentScripts $($Manifest.content_scripts.js) `
                            -ContentSecurityPolicy $($Manifest.content_security_policy) -Permissions $($Manifest.permissions)
                            $null = $OutputForObjectArray.Add($OutputSend)
                            $OutputSend
                        }
    ...
    $FirefoxInstalled = Get-ChildItem "\\$Computer\c$\Program Files\Mozilla Firefox\firefox.exe"
